...
export class ItemService {
  ...
  getItems(): Observable<AuctionItem[]> {
    ...
    return this.http
      .get<AuctionItem[]>(this.mockUrl)
      .pipe(tap((items) => localStorage.setItem(this.storageKey, JSON.stringify(items))));
  }

  getItemById(id: number): Observable<AuctionItem> {
    return this.getItems().pipe(map((items) => items.find((i) => i.ID === id) as AuctionItem));
  }
